...
Latest release builds (note: this repo is **private** — these links only work when you're logged into GitHub with access):
...
- **Arc** — the *same* [pr-impact-chrome-mv3.zip](https://github.com/0xdeafcafe/agenthub-ext/releases/latest/download/pr-impact-chrome-mv3.zip): unzip, then `arc://extensions` → Developer mode → Load unpacked.
- **Firefox** — [pr-impact-firefox-mv2.zip](https://github.com/0xdeafcafe/agenthub-ext/releases/latest/download/pr-impact-firefox-mv2.zip): `about:debugging` → This Firefox → Load Temporary Add-on → select the zip's `manifest.json`. Unsigned, so it installs as *temporary* (removed on browser restart) until published to AMO.
- **Safari** — [pr-impact-safari-xcode.zip](https://github.com/0xdeafcafe/agenthub-ext/releases/latest/download/pr-impact-safari-xcode.zip): an **unsigned** Xcode project generated by `safari-web-extension-converter`. Open it in Xcode, set your Apple ID under Signing & Capabilities, and build/run it yourself.

Every push to `main` also publishes these as workflow artifacts on the [Actions page](https://github.com/0xdeafcafe/agenthub-ext/actions/workflows/build.yml) (`pr-impact-chrome-arc`, `pr-impact-firefox`, `pr-impact-safari-xcode-project`).
